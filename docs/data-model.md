...
- `workout_sessions` — one per started workout: `programId`, `date`, `feeling`, `readiness` (1–5 pre-workout), `isCompleted`.
...
### Other tables
`exercise_prs` (PR detection), `user`/`session`/`account`/`verification`/`oauth_*` (better-auth), `friendships`/`program_shares`/`workout_reactions`/`nudges` (social), `ai_generations`/`ai_model_configs` (LLM program gen), `user_weight_entry`, `feedback`, `invite_token`.
...
Committed SQL in `drizzle/` (`0038` RIR, `0039` exercise-type, `0040` target-rir are the latest as of this writing). Workflow + idempotency rules are in [`CLAUDE.md`](../CLAUDE.md#database-migrations). Local-DB application gotcha: see [gotchas.md](gotchas.md).
