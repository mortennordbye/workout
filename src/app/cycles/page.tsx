 ...
 * Cycles Page
 *
 * Lists all training cycles grouped by status: active, draft, and past.
 */
...
function getWeekProgress(cycle: TrainingCycle): {
  currentWeek: number;
  progressPct: number;
} {
  ...
  const progressPct = Math.min(
    (days / (cycle.durationWeeks * 7)) * 100,
    100,
  );
...
export default async function CyclesPage() {
  ...
  const allCycles = result.success ? result.data : [];

  const active = allCycles.filter((c) => c.status === "active");
  const drafts = allCycles.filter((c) => c.status === "draft");
  const past = allCycles.filter((c) => c.status === "completed");
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col pb-24">
      ...
      <div className="flex items-center justify-between px-6 pt-8 pb-2">
        ...
        <Link
          href="/cycles/new"
          className="flex items-center justify-center w-10 h-10 rounded-full bg-primary text-primary-foreground active:opacity-80"
        >
        ...
        {drafts.length > 0 && (
          <div>
            ...
            <div className="flex flex-col divide-y divide-border rounded-2xl bg-muted overflow-hidden">
              {drafts.map((c) => (
                <DraftCycleRow key={c.id} cycle={c} />
              ))}
            </div>
          </div>
        )}
